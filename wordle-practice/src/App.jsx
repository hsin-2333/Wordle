import './App.css'
import { WordSheet } from './WordReducer'

function App() {

  return (
    <>
      <Grid className= 'text-3xl' grid= {initialState.grid} statusGrid={initialState.statusGrid}/>
    </>
  )
}
...
const Cell = ({value, status})=> (
  <div className={`cell ${colorStatus[status]}`}>
    {value}
  </div>
)
...
const initialState = {
  grid: Array(6).fill().map(() => Array(5).fill('')), // 只存 value
  statusGrid: Array(6).fill().map(() => Array(5).fill(letterStatus.idle)), // 存 status
  currentGuess: 0,
  answer: "APPLE",
};
